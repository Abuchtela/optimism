...
// defaultBootnodeStrings contains the raw enode strings for default bootnodes
// This avoids DNS resolution during package initialization
var defaultBootnodeStrings = []string{
	// OP Labs
...
// GetDefaultBootnodes returns the default bootnodes, parsing them at runtime
func GetDefaultBootnodes() []*enode.Node {
	nodes := make([]*enode.Node, 0, len(defaultBootnodeStrings))
	for _, s := range defaultBootnodeStrings {
		node, err := enode.Parse(enode.ValidSchemes, s)
		if err != nil {
			// Skip nodes that cannot be parsed (e.g., due to DNS resolution failures)
			log.Warn("Failed to parse bootnode", "enode", s, "err", err)
			continue
		}
...
// DefaultBootnodes is deprecated. Use GetDefaultBootnodes() instead to handle DNS resolution gracefully.
var DefaultBootnodes = GetDefaultBootnodes()
